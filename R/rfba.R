...
#'
#' @examples
fba.branches <- function(initial_d = 10,
                         showProgress = FALSE,
                         ...
                         #Angle parameters
                         orthoVertAnglesMaxDeg = 5,
                         plagioVertAnglesDeg = 85,
                         plagioVertAnglesSDDeg = 5,
                         twigVertAnglesDeg = 70,
                         twigVertAnglesSDDeg = 20
                         ) {
  set.seed(Randseed)
  if (showProgress)
    pb <- txtProgressBar(style = 3)

  orthoVertAnglesMax <- deg2rad(orthoVertAnglesMaxDeg)
  plagioVertAngles <- deg2rad(plagioVertAnglesDeg)
  plagioVertAnglesSD <- deg2rad(plagioVertAnglesSDDeg)
  twigVertAngles <- deg2rad(twigVertAnglesDeg)
  twigVertAnglesSD <- deg2rad(twigVertAnglesSDDeg)
  plagioDecr <- deg2rad(3)
  ...
  LengthD0 <-
    (InterceptStem + SlopeStem * initial_d) * (1 + RangeLStem * (runif(1) -
                                                                   0.5))
  ...
  branches[linkNumber, "Type"] <- 1
  branches[linkNumber, "VerAng"] <- 0
  branches[linkNumber, "RotAng"] <- 0
  branches[linkNumber, "TopX"] <- 0
  branches[linkNumber, "TopY"] <- LengthD0
  branches[linkNumber, "TopZ"] <- 0
  ...
  while (i <= length(DD)) {
    ...
    while (DD_child > DD_limit) {
      if (HowP == 0) {
        ...
        else if (RNumb < (Prob1P + Prob2P)) {
          P <- 0.5 + (runif(1) * 0.25)
        }
        else if (RNumb < (Prob1P + Prob2P + Prob3P)) {
          P <- 0.75 + (runif(1) * 0.15)
        }
        else if (RNumb < (Prob1P + Prob2P + Prob3P + Prob4P)) {
          P <- 0.9 + (runif(1) * 0.2)
        }
        else if (RNumb < (Prob1P + Prob2P + Prob3P + Prob4P + Prob5P)) {
          P <- 1.1 + (runif(1) * 0.15)
        }
        else if (RNumb < (Prob1P + Prob2P + Prob3P + Prob4P + Prob5P + Prob6P)) {
          P <- 1.25
        }
      } else {
        P <- 0
      ...
      if (is.null(BranchQ)) {
        ...
        else if (RNumb < (Prob1Q + Prob2Q)) {
          Q <- 0.6 + (runif(1) * 0.1)
        }
        else if (RNumb < (Prob1Q + Prob2Q + Prob3Q)) {
          Q <- 0.7 + (runif(1) * 0.1)
        }
        else if (RNumb < (Prob1Q + Prob2Q + Prob3Q + Prob4Q)) {
          Q <- 0.8 + (runif(1) * 0.1)
        }
        else if (RNumb < (Prob1Q + Prob2Q + Prob3Q + Prob4Q + Prob5Q)) {
          Q <- 0.9 + (runif(1) * 0.1)
        }
      } else {
    ...
    for (jsub in 1:Nsub)  {
      ...
      if (singleLD == 1) {
        Length <- (Intercept + Slope * (Dprox * DMin)) * (1 + RangeL * (runif(1) -
                                                                          0.5))
      } else if (Dprox < (MaxDiamTwig / DMin)) {
        Length <-
          (InterceptTwig + SlopeTwig * (Dprox * DMin)) * (1 + RangeLTwig * (runif(1) -
                                                                              0.5))
      } else if (Dprox < (MaxDiamBranch / DMin)) {
        Length <-
          (InterceptBranch + SlopeBranch * (Dprox * DMin)) * (1 + RangeLBranch *
                                                                (runif(1) - 0.5))
      } else {
        Length <-
          (InterceptWood + SlopeWood * (Dprox * DMin)) * (1 + RangeLWood * (runif(1) -
                                                                              0.5))
      }
      Length <- max(Length, 0)
      ...
      if (Dcur < Dzerofin) {
        if (Dcur < 1)
          FLength <- Length - LBaretip * ILatDmin
      ...
      if (jsub == 1) {
        branches[linkNumber, "Largest"] <- 1
        #Type -> ortho=1, plagio=2, twig=3
        #The biggest branches follow the parent type
        branches[linkNumber, "Type"] <- branches[linkParent, "Type"]
      } else {
        branches[linkNumber, "Type"] <- branches[linkParent, "Type"] + 1
      }


      #set the angle
      if (branches[linkNumber, "Type"] == 1) {
        #Orthotropic
        branches[linkNumber, "VerAng"] <- runif(1) * orthoVertAnglesMax
        branches[linkNumber, "RotAng"] <- runif(1) * 2 * pi
        do <- branches[linkNumber, "Diam1"]
        dn <- do
      } else if (branches[linkNumber, "Type"] == 2) {
        #Plagiotropic
        if(branches[linkParent, "Type"] == 1) {
          va <- rnorm(1, plagioVertAngles, plagioVertAnglesSD)
          branches[linkNumber, "VerAng"] <- va * (1 - branches[linkNumber, "Diam1"]/do)

          # rotAngStart <- rotAngStart + pi*branches[linkNumber, "Diam1"]/dn
          branches[linkNumber, "RotAng"] <- rotAngStart
          rotAngStart <- rotAngStart + pi*branches[linkNumber, "Diam1"]/dn
          dn <- branches[linkNumber, "Diam1"]

        } else {
          branches[linkNumber, "VerAng"] <- max(0, branches[linkParent, "VerAng"] - plagioDecr)
            # plagioDecr * (branches[linkParent, "VerAng"]/(pi*18))
          branches[linkNumber, "RotAng"] <- branches[linkParent, "RotAng"]
        }
      } else if (branches[linkNumber, "Type"] >= 3) {
        #Twig
        if(branches[linkNumber, "Type"] == branches[linkParent, "Type"]) {
          branches[linkNumber, "VerAng"] <- rnorm(1, branches[linkParent, "VerAng"], twigVertAnglesSD)
            # plagioDecr * (branches[linkParent, "VerAng"]/(pi*18))
          branches[linkNumber, "RotAng"] <- branches[linkParent, "RotAng"]
        } else {
          branches[linkNumber, "VerAng"] <- rnorm(1, twigVertAngles, twigVertAnglesSD)
          branches[linkNumber, "RotAng"] <- #runif(1) * 2 * pi
            rnorm(1, branches[linkParent, "RotAng"], twigVertAnglesSD)
        }
      }

      #calculate the top segment coordinate - for THREEJS
      branches[linkNumber, "TopY"] = branches[linkParent, "TopY"] +
        cos(branches[linkNumber, "VerAng"]) * Length
      branches[linkNumber, "TopX"] = branches[linkParent, "TopX"] -
        sin(branches[linkNumber, "VerAng"]) * Length * cos(branches[linkNumber, "RotAng"])
      branches[linkNumber, "TopZ"] = branches[linkParent, "TopZ"] +
        sin(branches[linkNumber, "VerAng"]) * Length * sin(branches[linkNumber, "RotAng"])

    }
    rotAngStart <- runif(1) * 2 * pi
    plagioVertAngles <- plagioVertAngles - plagioDecr
    ...
    if (showProgress)
      setTxtProgressBar(pb, i / length(DD))
    i <- i + 1
  }
  if (showProgress)
