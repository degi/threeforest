...
#' The object can be viewed using vtree().
#'
#' @param dbh Tree size
#' @param height
  ...
  if (is.data.frame(tree_df)) {
    ...
    for(i in 1:nrow(tree_df)) {
      t <- generate_tree(dbh = tree_df[i, "dbh"],
        ...
        t$x <- ifelse(is.blank(tree_df[i, "x"]), area$x_length * runif(1), tree_df[i, "x"])
        t$y <- ifelse(is.blank(tree_df[i, "y"]), area$y_length * runif(1), tree_df[i, "y"])
    ...
    for(i in 1:length(tree_df)) {
      ...
      t$x <- area$x_length * runif(1)
      t$y <- area$y_length * runif(1)
      t$id <- i
      f <- append(f, list(i = t))
    }
